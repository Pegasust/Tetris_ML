...
public:
    ...
    constexpr std::common_type_t<std::chrono::nanoseconds, std::chrono::nanoseconds> nano_time_diff(
        std::chrono::time_point<Clock> const& now) const {
        return now - then;
    }
    ...
    constexpr long long numeric_diff_nano(
        std::chrono::time_point<Clock> const& now = Clock::now()) const {
        auto d = nano_time_diff(now);
        return d.count();
    }
    constexpr long long numeric_diff_milli(
        std::chrono::time_point<Clock> const& now = Clock::now()) const {
    ...
    constexpr double fp_diff_milli(std::chrono::time_point<Clock> const& now = Clock::now()) const {
        ...
        return static_cast<double>(x) / static_cast<double>(std::chrono::nanoseconds::period::den /
                                                            std::chrono::milliseconds::period::den);
    }
    /*get time difference from nano in seconds*/
    constexpr double fp_diff_seconds(
        std::chrono::time_point<Clock> const& now = Clock::now()) const {
        long long x = numeric_diff_nano(now);
        return static_cast<double>(x) / static_cast<double>(std::chrono::nanoseconds::period::den);
    }
    /*get time difference from milliseconds to seconds*/
    constexpr double fp_diff_seconds_milli(
        std::chrono::time_point<Clock> const& now = Clock::now()) const {
        long long x = numeric_diff_milli(now);
            return static_cast<double>(x) / static_cast<double>(std::chrono::milliseconds::period::den);
    }
    /*get time difference from microseconds to seconds*/
    constexpr double fp_diff_seconds_micro(
        std::chrono::time_point<Clock> const& now = Clock::now()) const {
        long long x = (std::chrono::duration_cast<std::chrono::microseconds>(now - then)).count();
        return static_cast<double>(x) / static_cast<double>(std::chrono::microseconds::period::den);
    }
};
} // namespace GameClock
} // namespace Common