...
namespace TetrisAPI
{
	...
	constexpr TetrisEngine* new_game(const unsigned long long& seed)
	{
		return new Tetris::GameModule(seed);
	}

	constexpr void new_game(const unsigned long long& seed, TetrisEngine& mod)
	{
	...
	*/
	...
	(
		...
		// outputs
		bool& controlling_staticized, TetrisBody& controlling,
		unsigned char burn_y[4], unsigned char& n_burned
	)
	{
		game.update(input, burn_y, n_burned, false, delta_seconds,
			controlling_staticized, pre_static_threshold);
	...
	{
		TetrisAPI::update(game, input, DEFAULT_DELTA_TIME,
			DEFAULT_PRE_STATIC_THRESHOLD, controlling_staticized,
			controlling, burn_y, n_burned);
	}
	...
	constexpr void total_rows_burned(const TetrisEngine& game,
		unsigned long long& output)
	{
		output = game.n_rows_burned;
	}

	constexpr void total_tetris_scored(const TetrisEngine& game,
		unsigned int& output)
	{
		output = game.tetris_scored;
	}

	constexpr void times_reached_max_stage(const TetrisEngine& game,
	...
	format: actual rows being burned -> scaled rows
		...
		4 -> 5
	*/
	constexpr void scaled_rows_required(const TetrisEngine& game,
		unsigned long long& output)
	{
		output = game.n_rows;
	}

	constexpr void game_field(const TetrisEngine& game,
		TetrisField& output)
	{
