...
##    #pos_max = [range(pos[i], pos[i]+count[i]) for i in range(len(count))]
##    pos_max = [(pos[i] + count[i]) for i in range(len(count))]
    (min_pos, pos_max) = initialize(count)
    pos = min_pos[:]
    total_swap = 0
    i_sorted = [False]*len(unsorted)
##    print(">> pos_max: {}".format(pos_max))
...
##        while pos[unsorted[i]] != i:
        while not(i_sorted[i] or i >= min_pos[unsorted[i]] and i < pos_max[unsorted[i]]):
...
##                break
            ...
            if unsorted[swap_index] != unsorted[i]:
                # swap
##                print("swap({}): Swapping index {} to index {}".format(num_elements_sorted, i, swap_index))
                swap(unsorted, swap_index, i)
                total_swap += 1
            else:
                # The swap index is sorted, we don't need to swap it out of position, just
...
##    print("Total swap: {}".format(total_swap))
##    print(">> pos_max: {}".format(pos_max))
##    print(">> i_sorted: {}".format(i_sorted))
