...
template <
    typename FP_Type = double, typename Neuron_Type = NeuralNetwork::NeuronType,
...
public:
    ...
    public:
        ...
        // is_disabled
        bool is_disabled;
        inline bool is_recurrent() { return in_vertex_indx == out_vertex_indx; }
        inline bool disabled() { return is_disabled; }
    ...
    public:
        ...
        using ActivationFunc = FP_Type (*)(FP_Type, FP_Type);
        ...
        Vertex(
            Neuron_Type type, FP_Type activation_resposne = FP_(0), FP_Type last_output = FP_(0),
            ActivationFunc act_fn = Common::ZMath::sigmoid<FP_Type>) :
            neuron_type(type),
            response(activation_resposne), last_output(last_output), activation(act_fn)
        {
#ifndef NEURAL_NDEBUG
...
public:
    ...
    inline std::vector<Edge>& get_edges() { return edges; }
    inline const std::vector<Edge>& get_edges() const { return edges; }
    inline Index_Type vertices_count() { return vertices.size(); }
    inline Index_Type edges_count() { return edges.size(); }
    inline const Index_Type vertices_count() const { return vertices.size(); }
    inline const Index_Type edges_count() const { return edges.size(); }
    inline Edge& get_edge(const Index_Type edge_idx) { return edges[edge_idx]; }
    inline Vertex& get_vertex(const Index_Type vertex_idx) { return vertices[vertex_idx]; }
    inline const Edge& get_edge(const Index_Type edge_idx) const { return edges[edge_idx]; }
    ...
    inline Edge& vertex2edge(
        const Index_Type vertex_idx, const bool inwards, const Index_Type arrow_idx)
    {
    ...
    inline const Edge& vertex2edge(
        const Index_Type vertex_idx, const bool inwards, const Index_Type arrow_idx) const
    {
...
private:
    ...
    struct idx_of_type_functor {
        inline Index_Type operator()(
            const Index_Type hint, const Neuron_Type type,
...
public:
    static inline Index_Type get_vertex_id(
        Index_Type displacement, const Neuron_Type type, const TypeEndArray& arr)
    {
        if (type == NT_(0))
        {
            return displacement;
    ...
    static inline TypeEndArray make_end_array(
        Index_Type inputs = 3, Index_Type biases = 1, Index_Type hiddens = 0,
        Index_Type outputs = 1)
    {
        TypeEndArray arr;
...
public:
    ...
    inline Index_Type make_edge(
        const Index_Type from_vertex_idx, const Index_Type dest_vertex_idx,
    ...
    inline Index_Type make_vertex(
        const Neuron_Type neuron_type, const FP_Type activation_response = FP_(1),
    ...
    template <
        typename Vect_Resp, typename Vect_Out, bool reserve = true,
    ...
    inline void make_vertices(
        const TypeEndArray& type_end, const FP_Type default_activation_response = FP_(1),
        const FP_Type default_initial_output = FP_(0))
    {
        make_vertices(
            type_end, Common::TemplateArgs::no_args<>(), Common::TemplateArgs::no_args<>(),
    ...
    inline void set_input(
        const Index_Type input_disp, const FP_Type new_output, const TypeEndArray& type_end)
    {
...
public:
    ...
    template <
        typename Vect_In, typename Vect_Out, typename Start_Idx_Func,
    ...
    // output nodes.
    template <
        typename Vect_In, typename Vect_Out,
        bool parse_from_inputs = std::is_same<std::vector<FP_Type>, Vect_In>::value,
        bool extract_outputs = std::is_same<std::vector<FP_Type>, Vect_Out>::value>
